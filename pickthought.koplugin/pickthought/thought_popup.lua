...
pickthought/thought_popup/: this module is the public entry point; the widget
for the active position is pooled and reused across opens so reopening the
same thought rebuilds nothing.

@module pickthought.thought_popup
--]]
...
local function widgetClassFor(position)
    if position == "center" then
        return require("pickthought.thought_popup.center_widget")
    end
    return require("pickthought.thought_popup.widget")
end
...
-- 保留撷思旧调用面，避免其他阅读器扩展仍使用 snake_case 接口时失效。
M.close_visible = M.closeVisible
M.is_showing = M.isShowing
