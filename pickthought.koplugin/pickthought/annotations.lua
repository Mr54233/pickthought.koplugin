...
-- 容错子序列匹配(注释内联场景):本地 EPUB 转制时把脚注/尾注展开进了正文句中
-- (真机实证:"…人乳头状瘤病毒,这种…"→本地"…病毒(HPV),这种…"),跨插入点的
-- 划线整串 find 必失败。把微读原文视为有序字符序列,双指针推进:相同同进、
-- 不同只进本地指针(跳过插入物)。仅在整串匹配失败后触发,命中返回本地区间。
-- 护栏:needle 必须按序全命中;本地跳过量 <= max(8, #needle/3)——插入物
-- (注释/术语/数字)通常远短于原句,超过原文 1/3 的"匹配"是在硬串两段不同
-- 文字(真机反例:字面不同的两句 20 字中文靠公共字符窜过 24 字节上限)。
local FUZZY_ANCHOR_WINDOW = 200
local FUZZY_SKIP_FLOOR = 8

local function locate_subsequence(index, needle, expected)
    if not index or tostring(index.text or "") == "" or tostring(needle or "") == "" then
        return nil
    end
    local text = index.text
    -- 滑动前缀锚定:8→6→4 字节逐级缩短的 probe 在 expected±窗口内 find,
    -- 每级窗口未命中再全局 find。比首字节贪心稳定(中文首字节窗口内大量误中),
    -- 比固定 8 字节宽容(插入可能紧贴句首,长 probe 整个找不到)。仍全部失败
    -- 才退化为首字节锚,尽最大努力。
    local from = expected and math.max(1, expected - FUZZY_ANCHOR_WINDOW) or 1
    local to = expected and math.min(#text, expected + FUZZY_ANCHOR_WINDOW) or #text
    local pos = nil
    for _, len in ipairs({8, 6, 4}) do
        if #needle < len then break end
        local probe = needle:sub(1, len)
        pos = text:find(probe, from, true)
        if pos and pos <= to then break end
        pos = text:find(probe, 1, true)
        if pos then break end
    end
    if not pos then
        pos = text:find(needle:sub(1, 1), 1, true)
        if not pos then return nil end
    end
    local skip_limit = math.max(FUZZY_SKIP_FLOOR, math.floor(#needle / 3))
    local skipped = 0
    local j = 1
    i = pos
    while j <= #needle do
        if i > #text then return nil end
        if text:byte(i) == needle:byte(j) then
            i = i + 1
            j = j + 1
        else
            i = i + 1
            skipped = skipped + 1
            if skipped > skip_limit then return nil end
        end
    end
    local a = index.starts[pos]
    local b = index.ends[i - 1]
    if a ~= nil and b ~= nil and b > a then return a, b end
    return nil
end
...
local function intervals(data, visible_count, index)
    ...
    local stats = {quote_aligned=0, numeric=0, dropped=0, overlapped=0, unlocated=0, fuzzy=0}
    for _, row in ipairs(data.underlines or {}) do
        ...
        if raw_a then
            ...
            if not a then
                -- 整串失败 → 容错子序列(注释内联插入)。命中区间来自本地文本,
                -- 不依赖数字偏移,合并文件(no_numeric_fallback)场景同样适用。
                for _, quote in ipairs(quote_candidates(row, data)) do
                    a, b = locate_subsequence(index, quote, raw_a)
                    if a then break end
                end
                if a then stats.fuzzy = stats.fuzzy + 1 end
            end
            if a then
...
local function annotation_stats(data, alignment)
    return {
        ...
        fuzzy=alignment and alignment.fuzzy or 0,
