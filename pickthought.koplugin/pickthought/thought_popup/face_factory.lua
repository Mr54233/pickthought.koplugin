...
function FaceFactory:findEmojiFont()
    ...
    local function resolve(path)
        ...
        if ok_ffiutil and ffiutil.realpath then
    ...
    for _, path in ipairs(candidates) do
        ...
        if abs then
            self.emoji_path = abs
            logger.info("[撷思][ThoughtFace] emoji fallback:", abs)
...
function FaceFactory:_buildFace(path, size)
    ...
    if not ok or not ftsize then
        logger.warn("[撷思][ThoughtFace] failed to open font:", path, ftsize)
        return nil
    end
    local face_obj = {
        orig_font = path,
...
function FaceFactory:getFace(doc_font_name, size, variant)
    ...
    -- 极少数裁剪版 KOReader 没有被 FontList 扫描到内置字体时，仍用其标准
    -- FontFaceObj 兜底。此路径不改全局 fallback 表，Emoji 链只在上方自建
    -- face 成功时挂载。
    if not face then
