...
local CURRENT_SYNC_MIN_AVAILABLE_KB=96*1024
...
function Store:preferences()
    ...
    -- 96MB 是 worker 运行中的安全线,不是用户可配置选项。此前版本曾把
    -- 128MB 与 fork 门槛共用,升级时迁移回独立的 96MB 运行中下限。
    if tonumber(preferences.sync_min_available_kb)==PREVIOUS_SYNC_MIN_AVAILABLE_KB then
