...
local CURRENT_SYNC_MIN_AVAILABLE_KB=72*1024
...
function Store:preferences()
    ...
    -- 72MB 是 worker 运行中的安全线,不是用户可配置选项。128MB 是 fork 前门槛,
    -- 两者独立；升级时把历史 128/96MB 运行线迁移到新的 72MB。
    if tonumber(preferences.sync_min_available_kb)==PREVIOUS_SYNC_MIN_AVAILABLE_KB
        or tonumber(preferences.sync_min_available_kb)==PREVIOUS_WORKER_SYNC_MIN_AVAILABLE_KB then
